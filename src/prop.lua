...
__props_inst_mt = {
	__newindex = function (p, key, val)
		...
		local vvaltype = kontype(validvalue)
		if row.type ~= nil and row.type ~= vvaltype then
			error("property [" .. tostring(key) .. "] is a " .. row.type .. ", tried to assign a " .. kontype(val)
			       .. " (" .. tostring(val) .. ")")
		end
	...
	__index = function (p, key)
		...
		local proto = rawget(p, "__proto")
		if v ~= nil then
			--if kontype(v) == "function" and p.__proto[key].callonly then
			--if type(v) == "function" and p.__proto[key].callonly then
			--	return v()
			--elseif kontype(v) == "link" then
			--	return v[1]()
			--else
			--	return v
			--end
...
Props.__call = function (proto, init)
	...
	for key,row in pairs(proto) do
		...
		rawset(props, key, init_value or proto[key].default)
